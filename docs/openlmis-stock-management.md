...
## Mediator SOH safety gate

The mediator enforces a stock-on-hand check **before** every dispense or adjustment fans out to downstream systems. The logic lives in [`src/integrations/openlmis.js`](../mediator/src/integrations/openlmis.js) (`checkStock`) and is called from [`src/fanout.js`](../mediator/src/fanout.js) (`executeFanout`).

### Flow

```
POST /fhir/MedicationDispense  (or QuestionnaireResponse type=DISPENSE/ADJUSTMENT)
  │
  ├─▶ checkStock(identity, qty)
  │     GET /api/stockCardSummaries?facility=…&program=…&orderable=…
  │     ├─ SOH ≥ qty  →  { ok: true,  stockOnHand: N }
  │     ├─ SOH < qty  →  { ok: false, stockOnHand: N, requested: qty }
  │     └─ error      →  { ok: true }   ← FAIL-OPEN (patient care is never blocked)
  │
  ├─ ok=false  →  HTTP 422  { status: "Rejected", reason: "insufficient-stock", stockOnHand, requested }
  └─ ok=true   →  fan-out proceeds (OpenSRP + OpenLMIS + DHIS2)
```

**Receipts bypass the gate** — `isReceipt=true` skips `checkStock` entirely (adding stock cannot be insufficient).

**Orders bypass the gate** — `type=ORDER` (stock replenishment request) is buffered to DHIS2 and never hits the SOH gate; the response includes the current SOH for informational purposes.

### Low-stock notifications

After a successful dispense, if the projected new SOH (`stockOnHand - qty`) falls below `NOTIFY_LOW_STOCK_THRESHOLD` (default `20`) and `NOTIFY_ON_LOW_STOCK=true`, the mediator fires a notification to the VHW.

### Background SOH poll

A background loop (default every 60 s, configurable via `POLL_INTERVAL_MS`) calls `pollStockLevels()` in [`src/poller.js`](../mediator/src/poller.js). If it detects an increase in SOH that did not come through the mediator (i.e. stock received directly in OpenLMIS), it records the delta as a stock-received data value in DHIS2.

### Insufficient-stock example

```bash
# Attempt to dispense more than is available
curl -s -X POST http://localhost:5001/fhir/MedicationDispense \
  ...
  -d '{
    ...
    "quantity":     {"value": 99999, "unit": "tablet"}
  }'
```
