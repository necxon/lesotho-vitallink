...
| [backup-and-restore.md](backup-and-restore.md) | Backup and disaster recovery on a single server - what is in an archive, the nightly schedule and retention, the portal's Backup & Recovery page, getting a copy off the machine, the restore procedure, and `make backup-verify` to prove an archive really restores |
| [phone-menus-and-forms.md](phone-menus-and-forms.md) | The config chain a phone walks at startup, why the side menu went from 20 items to 6, why per-role menus do not work on the installed app, the LHC-Forms form preview, and `make test-menus` to check every menu item and form end to end |
| [superset-reporting-requirements.md](superset-reporting-requirements.md) | For sign-off: the five dashboard category views with their indicators and audiences, the five custom operational reports with users/frequency/outputs, the KPI-to-data-source matrix, and what has to be populated before the unpopulated KPIs can be agreed |
