...
| UID | Name | Parent | Notes |
|-----|------|--------|-------|
| `dwx1Yz4BwNX` | Maseru District Clinic A | Maseru District | Facility-level — dispense/receipt/SOH |
| `VilHaMokoe1` | Ha Mokoena Village | Maseru District Clinic A | VHW: Thabo Mokoena. GPS `[27.523, -29.354]` |
| `VilHaSehl01` | Ha Sehlabane Village | Maseru District Clinic A | VHW: Lineo Nthabi. GPS `[27.441, -29.283]` |
| `VilMatsien1` | Matsieng Village | Maseru District Clinic A | VHW: Mpho Lerotholi. GPS `[27.552, -29.421]` |

### Data Elements — Stock Integration (dhis2-integration)

The standard OpenLMIS→DHIS2 integration uses **one data element per product**, differentiated by category option combo per indicator type. Written by the `dhis2-integration` service (not the mediator directly).

| UID | Name | aggregationType | Written by |
|-----|------|-----------------|------------|
| `ALStockDE01` | AL 20/120mg | SUM | `dhis2-integration` on sync |

#### Category Combo: BKM Stock Indicators (`BKMCatComb1`)

| Category option combo UID | Name | Indicator |
|---------------------------|------|-----------|
| `oop5xfCVJ01` | Negative Adjustments | Dispenses + negative stock adjustments |
| `R988AFoArQU` | Positive Adjustments | Receipts |
| `E8qivaMehZx` | Closing Balance | Stock on hand at period end |

> Sync is triggered manually via the **Sync Now** button on bkm-web Services page, or by `POST /mediator-api/dhis2/sync`. The dhis2-integration service reads OpenLMIS `stockCardSummaries` and posts to `POST /api/dataValueSets?orgUnitIdScheme=code&dataElementIdScheme=name&categoryOptionComboIdScheme=name`.

### Data Elements — Order Buffer (mediator direct)

| UID | Name | aggregationType | Written by |
|-----|------|-----------------|------------|
| `DEOrdAL0001` | Stock Ordered — AL 20/120mg | SUM | `orderBuffer.addOrder()` at village org unit |
| `DEOrdAmx001` | Stock Ordered — Co-Amoxiclav 400/57mg | SUM | `orderBuffer.addOrder()` |
| `DEOrdRdt001` | Stock Ordered — RDT Kit | SUM | `orderBuffer.addOrder()` |
| `DEOrdPar001` | Stock Ordered — Paracetamol 500mg | SUM | `orderBuffer.addOrder()` |
| `DEOrdCtx001` | Stock Ordered — CTX 480mg | SUM | `orderBuffer.addOrder()` |
| `DEOrdOrs001` | Stock Ordered — ORS Sachets | SUM | `orderBuffer.addOrder()` |
| `DEOrdZnc001` | Stock Ordered — Zinc Sulfate 20mg | SUM | `orderBuffer.addOrder()` |
| `DEOrdIfa001` | Stock Ordered — Ferrous+Folic | SUM | `orderBuffer.addOrder()` |

> Order DEs are written at the **village org unit** level (one per VHW), not the facility level. The dispatch aggregates across all villages when sending to OpenLMIS.

### Dashboards

| UID | Name | Contents | URL |
|-----|------|----------|-----|
| `BKMStkDsh01` | BKM Stock — AL 20/120mg | SOH line, dispensed bar, received bar, all-indicators pivot | [open](http://localhost:8081/dhis-web-dashboard/index.html#/BKMStkDsh01) |
| `BKMOrdDsh01` | BKM Stock Order Status | Live text: DISPATCHED/PENDING, next run, order flow | [open](http://localhost:8081/dhis-web-dashboard/index.html#/BKMOrdDsh01) |
| `BKMOrdInfo1` | BKM Stock Order System — How It Works | Static explanation: workflow, monitoring, retry guide | [open](http://localhost:8081/dhis-web-dashboard/index.html#/BKMOrdInfo1) |
| `BKMVhwMap01` | BKM VHW Stock Orders — Village Map | Bubble map + pivot village×medicine + bar chart | [open](http://localhost:8081/dhis-web-dashboard/index.html#/BKMVhwMap01) |

> `BKMStkDsh01` data updates when you click **Sync Now** on the bkm-web Services page or call `POST /mediator-api/dhis2/sync`. After syncing, run DHIS2 analytics (`POST /api/resourceTables/analytics`) to refresh chart data.
> `BKMOrdDsh01` text is updated automatically by the mediator after every dispatch attempt and on startup.
